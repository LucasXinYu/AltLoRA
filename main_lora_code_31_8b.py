...
def main():
    ...
    if data_args.dataset_name is not None:
        # Load Code-Feedback dataset from Hugging Face
        ...
        split_datasets = full_dataset["train_sft"].train_test_split(
    ...
    if model_args.config_name:
        ...
        config = AutoConfig.from_pretrained("./checkpoints/Llama-3.1-8B",**{k: v for k, v in config_kwargs.items() if k != "rope_scaling"})
    else:
    ...
    elif any(v in model_args.model_name_or_path.lower() for v in ["llama-3", "llama-3.1", "llama-3.2"]) and "instruct" not in model_args.model_name_or_path.lower():
    ...
    def tokenize_function(examples):
        ...
        tokenized_full = tokenizer(full_texts, truncation=True, padding=False, max_length=2048)
        tokenized_prompt = tokenizer(prompt_texts, truncation=True, padding=False, max_length=2048)

        labels = []
        for full_ids, prompt_ids in zip(tokenized_full["input_ids"], tokenized_prompt["input_ids"]):
            lab = full_ids[:len(full_ids)].copy()
            cutoff = min(len(prompt_ids), len(lab))
    ...
    if training_args.do_eval:
        ...
        def compute_metrics(eval_preds):
            ...
            return {"simple_accuracy": correct / total}
    train_loss = []
    val_loss = []
    ...
    for round in tqdm(range(fed_args.num_rounds)):
        ...
        if data_args.dataset_name == "TIGER-Lab/CodeFeedback":
            pass
        else: 
            if training_args.do_eval:
