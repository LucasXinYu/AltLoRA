...
class altlora_v2(Optimizer):
    def __init__(self, model, lr=1e-3, betas=(0.999, 0.98), eps=1e-6, weight_decay=1e-5, correct_bias=True, rank=2, reg=1e-3):
        ...
        params = list(model.parameters())
        defaults = dict(lr=lr, betas=betas, eps=eps, weight_decay=weight_decay, correct_bias=correct_bias)
        super(altlora_v2, self).__init__(params, defaults)
        self.model = model
        self.rank = rank
        self.reg = reg
    def reset_state(self):
                ...
                if i + 1 < len(named_params):
                    ...
                    if p1.grad is not None or p2.grad is not None:
                        ...
                        if p1.grad is not None:
                            ...
                            p1.data = p1.data - step_size * (beta1 * P_at @ exp_avg + (1-beta1) * grad1_scaled)
                        ...
                        if p2.grad is not None:
                            ...
                            p2.data = p2.data - step_size * (beta1 * exp_avg @ P_bt + (1-beta1) * grad2_scaled) 
                ...
                if i + 1 < len(named_params):
                    ...
                    if p1.grad is not None or p2.grad is not None:
                        if p1.grad is not None:
                            ...
                            S =  exp_avg_sq_root @  grad1_scaled 
